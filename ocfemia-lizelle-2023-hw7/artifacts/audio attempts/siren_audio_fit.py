...
if __name__ == "__main__":
    ...
    def get_aud(aud_path):
        ...
        waveform = tf.audio.decode_wav(aud_raw)
        return (
            get_mgrid(len(waveform.audio), 1),
            waveform.audio,
    ...
    aud_mask, aud_train, sample_rate = get_aud('mwahahahaha.wav')
    
    audio_siren = Siren(in_features=1, out_features=1, hidden_features=256, hidden_layers=3, 
                        first_omega_0=3000, outermost_linear=True)
    ...
    for i in bar:
        ...
        grads = tape.gradient(loss, audio_siren.trainable_variables)
        optimizer.apply_gradients(grads, audio_siren.trainable_variables)
    ...
    predicted_audio = tf.audio.encode_wav(100*y_hat, sample_rate)
    fig.savefig('preds.pdf')
    breakpoint()

    tf.io.write_file('pred.wav',predicted_audio)
